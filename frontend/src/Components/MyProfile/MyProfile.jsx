    ...
    const fetchData = async () => {
      try {
        ...
        if (token) {
          ...
          if (userId) {
            // Fetch user info
            const response = await axios.get(`https://aarshi.onrender.com/api/users/${userId}`, {
              headers: {
            ...
            const ordersResponse = await axios.get('https://aarshi.onrender.com/api/orders/user-orders', {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
